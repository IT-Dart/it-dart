...
export default function AdminScreen({onClose}){
  ...
  const [toolOverview,setToolOverview]=useState(null); // Fixkosten+ai-chat-Kosten je registriertem Tool (Admin-only)
  ...
  const loadCostSummary=async()=>{
    ...
    if(error){console.error("[AdminScreen] loadCostSummary failed:",error.message);setCostSummary([]);return;}
    setCostSummary(data||[]);
  };
  const loadToolOverview=async()=>{
    const {data:tools,error:tErr}=await supabase.from("tools").select("*").order("name");
    if(tErr){console.error("[AdminScreen] loadToolOverview failed:",tErr.message);setToolOverview([]);return;}
    const {data:costs,error:cErr}=await supabase.rpc("get_tool_cost_overview");
    if(cErr){console.error("[AdminScreen] get_tool_cost_overview failed:",cErr.message);}
    const costBySlug=Object.fromEntries((costs||[]).map(c=>[c.slug,c]));
    setToolOverview((tools||[]).map(t=>({...t,cost:costBySlug[t.slug]||null})));
  };
  useEffect(()=>{loadTrainerList();loadAllUsers();loadCostSummary();if(isAdmin)loadToolOverview();},[isAdmin]);
  ...
  return(
    <div style={wrap}><div style={inner}>
      ...
      {isAdmin&&<div style={{background:C.s1,border:`0.5px solid ${C.bd}`,borderRadius:12,padding:"14px 16px",marginBottom:24}}>
