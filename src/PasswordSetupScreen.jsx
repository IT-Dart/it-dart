...
// Zeigt sich nach dem allerersten Login über einen vom Trainer manuell
// weitergegebenen Magic-Link (Trainee-Onboarding ohne E-Mail, To-Do #67) --
// so ein Konto hat nie ein eigenes Passwort gesetzt und würde sonst bei
// jedem weiteren Login wieder einen frischen Link vom Trainer brauchen.
...
export default function PasswordSetupScreen(){
  ...
  if(done)return(
    <div style={wrap}><div style={{...inner,paddingTop:60,textAlign:"center"}}>
      ...
      <h2 style={{fontSize:20,fontWeight:700,marginBottom:8}}>Passwort eingerichtet</h2>
      <p style={{fontSize:14,color:C.t2}}>Du kannst dich ab jetzt jederzeit mit deinem eigenen Passwort anmelden. Du wirst gleich automatisch weitergeleitet...</p>
    </div></div>
  );

  return(
    <div style={wrap}><div style={{...inner,paddingTop:60}}>
      <h2 style={{fontSize:22,fontWeight:700,marginBottom:6}}>Passwort einrichten</h2>
      <p style={{fontSize:13,color:C.t2,marginBottom:24}}>Du bist gerade über einen Link deines Trainers angemeldet. Damit du dich künftig auch ohne neuen Link anmelden kannst, lege jetzt einmalig ein eigenes Passwort fest.</p>
      <form onSubmit={submit}>
        ...
        <button type="submit" disabled={busy} style={{...pri,width:"100%",justifyContent:"center",opacity:busy?.6:1}}>
          {busy?"Bitte warten...":"Passwort einrichten →"}
        </button>
      </form>
    </div></div>
  );
