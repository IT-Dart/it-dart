...
import RechentrainerScreen from "./RechentrainerScreen";
...
export default function ITDart({onOpenExam,onOpenLegal,wartungsmodus}){
  ...
  const {user,isPremium,premiumUntil,isRechentrainerUnlocked,isAdmin,isTrainer,isJuniorAdmin,signOutAndRedirect,signingOut}=useAuth();
  ...
  if(view==="rechentrainer")return user?<RechentrainerScreen onClose={()=>changeView("overview")}/>:null;
  ...
  if(view==="overview")return(
    <div style={wrap}><div style={inner}>
      ...
      <p style={{fontSize:11,fontWeight:600,letterSpacing:".06em",textTransform:"uppercase",color:C.mu,marginBottom:8}}>Zusatzwerkzeug</p>
      <div style={{marginBottom:28}}>
        <button onClick={()=>changeView("rechentrainer")} style={{display:"flex",alignItems:"center",gap:12,textAlign:"left",width:"100%",background:"#1a2535",border:`0.5px solid ${C.bd}`,borderRadius:10,padding:"12px 14px",cursor:"pointer",color:"inherit",fontFamily:"inherit"}}>
          <span style={{fontSize:22,flexShrink:0}}>🧮</span>
          <span style={{flex:1}}>
            <span style={{display:"block",fontSize:14,fontWeight:600,color:C.t}}>Rechentrainer</span>
            <span style={{display:"block",fontSize:12,color:C.mu,marginTop:2}}>Unbegrenzt Subnetting-Aufgaben mit sofortigem Feedback</span>
          </span>
          <span style={{fontSize:11,padding:"2px 8px",borderRadius:4,fontWeight:500,background:isRechentrainerUnlocked?"#14532d":"#3a2a0f",color:isRechentrainerUnlocked?"#86efac":"#fbbf24",whiteSpace:"nowrap"}}>{isRechentrainerUnlocked?"Verfügbar":"🔒 Zusatzkauf"}</span>
        </button>
      </div>
