...
const GQ=[
  ...
  {q:"Warum rechnen Computer im Binärsystem?",o:["Einfacher zu lernen","Dezimal zu langsam","Schaltkreise kennen nur zwei Zustände","Hex zu komplex"],c:2,e:"Transistoren kennen nur Strom-fließt (1) oder nicht (0). Binär ist die natürliche Sprache der Hardware."},
  ...
  {q:"Welche Aufgabe hat das Netzteil im PC?",o:["Daten dauerhaft speichern","230-V-Wechselstrom in Gleichspannungen für die Komponenten wandeln","Die CPU kühlen","Das Betriebssystem laden"],c:1,e:"Das Netzteil wandelt die Netzspannung in die Gleichspannungen (z.B. 12 V, 5 V, 3,3 V) um, mit denen Mainboard, CPU und Laufwerke arbeiten."},
...
const BQ=[
  {q:"Hauptaufgabe eines Betriebssystems?",o:["Dokumente erstellen","Schnittstelle zwischen Hardware und Anwendungen","Bildschirm ansteuern","Netzwerkkabel verwalten"],c:1,e:"Das OS verwaltet alle Hardware-Ressourcen und stellt Anwendungen eine einheitliche Schnittstelle zur Verfügung."},
  ...
  {q:"Hauptvorteil von PowerShell gegenüber cmd?",o:["Hat eine GUI","Gibt Objekte zurück und ist skriptfähig","Schneller beim Tippen","Funktioniert ohne Windows"],c:1,e:"PowerShell gibt strukturierte Objekte zurück die man weiterverarbeiten kann. Befehle laufen auf vielen Servern gleichzeitig."},
  ...
  {q:"Was zeigt der Befehl ipconfig unter Windows?",o:["Laufende Prozesse","Die IP-Konfiguration der Netzwerkadapter","Installierte Programme","Offene Ports"],c:1,e:"ipconfig zeigt IP-Adresse, Subnetzmaske und Gateway – oft der erste Schritt bei Netzwerkproblemen."},
  {q:"Was unterscheidet einen Dienst von einem normalen Programm?",o:["Nichts","Er läuft im Hintergrund, auch ohne angemeldeten Benutzer","Er läuft nur unter Linux","Er braucht immer ein Fenster"],c:1,e:"Ein Dienst (Service) startet meist mit dem System und läuft ohne Benutzeranmeldung – z.B. der Druckspooler."},
  {q:"Welchen Vorteil bietet NTFS gegenüber FAT32?",o:["Nur für USB-Sticks geeignet","Zugriffsrechte, große Dateien und höhere Ausfallsicherheit","Schnelleres Internet","Bessere Grafikleistung"],c:1,e:"NTFS unterstützt Berechtigungen, Dateien über 4 GB und ist robuster gegen Abstürze – Standard für Windows-Systeme."},
  {q:"Welche Aufgabe hat der Task-Manager?",o:["Dateien kopieren","Prozesse und Auslastung anzeigen und beenden","Treiber installieren","Netzwerke einrichten"],c:1,e:"Der Task-Manager zeigt laufende Prozesse sowie CPU- und RAM-Auslastung und kann hängende Programme beenden."},
];
...
const SIQ=[
  ...
  {q:"Was ist Social Engineering?",o:["Netzwerkangriff","Manipulation von Menschen statt Systemen","Verschlüsselungsmethode","Firewall-Umgehung"],c:1,e:"Social Engineering nutzt menschliche Schwächen statt technischer Lücken."},
  {q:"Was unterscheidet asymmetrische von symmetrischer Verschlüsselung?",o:["Asymmetrisch ist schneller","Symmetrisch hat zwei Schlüssel","Asymmetrisch hat Schlüsselpaar: öffentlich und privat","Symmetrisch ist sicherer"],c:2,e:"Asymmetrisch: öffentlicher Schlüssel zum Verschlüsseln, privater zum Entschlüsseln. Symmetrisch: ein Schlüssel für beide Seiten."},
  ...
  {q:"Was macht eine Firewall?",o:["Daten verschlüsseln","Netzwerkverkehr überwachen und blockieren","Viren erkennen","Backups erstellen"],c:1,e:"Eine Firewall analysiert Netzwerkpakete nach Regeln und blockiert unerlaubten Verkehr."},
  {q:"Was versteht man unter Tailgating?",o:["Eine Art Netzwerkangriff","Sich hinter einem Berechtigten durch eine Sicherheitsschleuse schleusen","Ein Phishing-Angriff per E-Mail","Passwörter über die Schulter ausspähen"],c:1,e:"Tailgating ist eine physische Bedrohung: Eine unbefugte Person nutzt die geöffnete Tür einer berechtigten Person um in gesicherte Bereiche zu gelangen — ohne eigenen Ausweis."},
  {q:"Was ist Zwei-Faktor-Authentifizierung (2FA)?",o:["Zwei Passwörter nacheinander","Anmeldung mit zwei Faktoren, z.B. Passwort plus Code vom Handy","Doppelte Verschlüsselung","Zwei Benutzerkonten"],c:1,e:"2FA kombiniert Wissen (Passwort) mit Besitz (z.B. Smartphone-Code). Ein gestohlenes Passwort allein reicht dann nicht mehr."},
  {q:"Was ist Ransomware?",o:["Werbesoftware","Schadsoftware, die Daten verschlüsselt und Lösegeld fordert","Ein Passwort-Manager","Eine Firewall-Art"],c:1,e:"Ransomware verschlüsselt Dateien und erpresst Lösegeld – wichtigster Schutz sind aktuelle, getrennt aufbewahrte Backups."},
  {q:"Was versteht man unter Shoulder Surfing?",o:["Über WLAN mitlesen","Beim Eintippen von Passwörtern über die Schulter schauen","Phishing per Telefon","Einen Web-Filter"],c:1,e:"Shoulder Surfing ist das Ausspähen von Eingaben durch direktes Zusehen – etwa im Zug oder im Großraumbüro."},
  {q:"Wozu dient ein VPN?",o:["Schnelleres Internet","Ein verschlüsselter Tunnel durch unsichere Netze","Virenschutz","Automatische Backups"],c:1,e:"Ein VPN baut einen verschlüsselten Tunnel auf – z.B. für den sicheren Zugriff aufs Firmennetz aus dem Homeoffice."},
];
...
const DBQ=[
  {q:"Hauptunterschied SQL vs NoSQL?",o:["SQL ist schneller","SQL speichert in Tabellen mit Beziehungen, NoSQL ist flexibler","NoSQL hat keine Abfragen","SQL kann nur eine Tabelle"],c:1,e:"SQL: strukturierte Tabellen mit Beziehungen. NoSQL: flexibel, für große unstrukturierte Datenmengen."},
  ...
  {q:"Zweck der Normalisierung?",o:["Datenbank verschlüsseln","Redundanz vermeiden und Integrität gewährleisten","Abfragen beschleunigen","Benutzerrechte verwalten"],c:1,e:"Normalisierung beseitigt Redundanz. Eine Adresse nur einmal gespeichert = nur einmal ändern nötig."},
  {q:"Was ist eine REST-API?",o:["Datenbank für Web","Schnittstelle mit HTTP-Methoden für Datenaustausch","Verschlüsselungsprotokoll","Datenbankformat"],c:1,e:"REST-API nutzt HTTP: GET, POST, PUT, DELETE. Programme kommunizieren unabhängig von Sprache oder Plattform."},
  {q:"Wozu dient ein Fremdschlüssel?",o:["Er verschlüsselt die Tabelle","Er verknüpft Datensätze zweier Tabellen","Er sortiert die Ergebnisse","Er löscht Duplikate"],c:1,e:"Ein Fremdschlüssel verweist auf den Primärschlüssel einer anderen Tabelle und stellt so Beziehungen zwischen Tabellen her."},
  ...
  {q:"Was bewirkt die WHERE-Klausel?",o:["Sie sortiert Ergebnisse","Sie filtert Zeilen nach Bedingungen","Sie verbindet Tabellen","Sie zählt Datensätze"],c:1,e:"WHERE schränkt ein, welche Zeilen betroffen sind – bei SELECT genauso wie bei UPDATE und DELETE."},
  {q:"Was besagt die 3-2-1-Backup-Regel?",o:["3 Admins, 2 Server, 1 Standort","3 Kopien, auf 2 Medientypen, davon 1 extern","3 Backups pro Tag","3 verschiedene Passwörter"],c:1,e:"3 Kopien der Daten, auf 2 verschiedenen Medien, davon 1 außer Haus – schützt auch bei Brand oder Ransomware."},
  {q:"Warum ist Redundanz in Tabellen problematisch?",o:["Sie spart Speicherplatz","Mehrfach gespeicherte Daten werden bei Änderungen leicht inkonsistent","Sie beschleunigt Abfragen","Sie ist gesetzlich vorgeschrieben"],c:1,e:"Steht dieselbe Information mehrfach, wird bei Änderungen leicht eine Stelle vergessen – die Normalisierung verhindert genau das."},
];
...
const SKQ=[
  {q:"Was ist der Hauptunterschied zwischen Skript und kompiliertem Programm?",o:["Skripte sind schneller","Skripte werden interpretiert, Programme in Maschinencode kompiliert","Programme sind einfacher zu schreiben","Skripte laufen nur auf Windows"],c:1,e:"Skripte werden Zeile für Zeile zur Laufzeit interpretiert. Kompilierte Programme werden vorher in Maschinencode übersetzt — schneller aber weniger flexibel."},
  ...
  {q:"Was ist ein Cronjob unter Linux?",o:["Ein dauerhaft laufender Prozess","Eine automatisch geplante Aufgabe die zu bestimmten Zeiten läuft","Ein Backup-Tool","Ein Admin-Benutzer"],c:1,e:"Cron ist ein Zeitplaner unter Linux. Cronjobs legen fest wann Skripte oder Befehle automatisch ausgeführt werden."},
  ...
  {q:"Was bewirkt die Pipe | in PowerShell und Bash?",o:["Sie kommentiert eine Zeile aus","Sie reicht die Ausgabe eines Befehls als Eingabe an den nächsten weiter","Sie startet einen Befehl doppelt","Sie bricht das Skript ab"],c:1,e:"Die Pipe verbindet Befehle zu einer Kette – in PowerShell fließen dabei Objekte, in Bash Text."},
  {q:"Wozu dient eine foreach-Schleife?",o:["Einen Codeblock für jedes Element einer Liste ausführen","Fehler abfangen","Dateien löschen","Variablen definieren"],c:0,e:"foreach wiederholt den Codeblock für jedes Element – z.B. denselben Check nacheinander für 50 Server."},
  {q:"Was macht grep unter Linux?",o:["Dateien kopieren","Textzeilen nach einem Muster filtern","Prozesse beenden","Rechte ändern"],c:1,e:"grep durchsucht Text zeilenweise nach Mustern – klassisches Beispiel: ps aux | grep nginx."},
  {q:"Welches Windows-Werkzeug entspricht dem Linux-Cron?",o:["Task-Manager","Aufgabenplanung (Task Scheduler)","Ereignisanzeige","Explorer"],c:1,e:"Die Aufgabenplanung startet Programme und Skripte zeitgesteuert – das Windows-Pendant zum Cron-Daemon."},
  {q:"Was ist eine Variable in einem Skript?",o:["Ein fester Befehl","Ein benannter Speicherplatz für Werte","Eine Datei","Ein Kommentar"],c:1,e:"Variablen wie $server speichern Werte, die das Skript mehrfach verwendet oder unterwegs verändert."},
];
...
const PRQ=[
  {q:"Was ist der Unterschied zwischen 1st und 2nd Level Support?",o:["1st Level ist teurer","1st Level löst einfache Probleme direkt, 2nd Level übernimmt komplexere Fälle","2nd Level hat weniger Erfahrung","1st Level darf keine Tickets erstellen"],c:1,e:"1st Level (Helpdesk) nimmt Anfragen entgegen und löst einfache Probleme direkt. Was nicht lösbar ist wird an 2nd Level eskaliert — spezialisierte Admins mit mehr Rechten."},
  {q:"Was ist ein SLA?",o:["Ein Softwarelizenzvertrag","Eine Vereinbarung über Reaktions- und Lösungszeiten bei Problemen","Ein Sicherheitszertifikat","Ein Backup-Plan"],c:1,e:"Ein Service Level Agreement legt fest wie schnell auf Probleme reagiert werden muss. Kritische Störungen z.B. Reaktion innerhalb 1 Stunde, normale Anfragen innerhalb 24 Stunden."},
  {q:"Was ist eine Bedarfsanalyse?",o:["Eine technische Systemprüfung","Eine Sicherheitsanalyse","Das Klären was der Kunde wirklich braucht bevor man eine Lösung anbietet","Ein Protokoll nach Projektabschluss"],c:2,e:"Erst verstehen, dann lösen. Die Bedarfsanalyse klärt IST-Zustand und SOLL-Zustand. Ohne Bedarfsanalyse löst man oft das falsche Problem."},
  {q:"Was enthält ein Abnahmeprotokoll?",o:["Den Quellcode der Software","Eine Bestätigung dass die Leistung den vereinbarten Anforderungen entspricht","Den Stundennachweis des Technikers","Die Rechnung"],c:1,e:"Das Abnahmeprotokoll bestätigt dass alle vereinbarten Leistungen erbracht wurden. Mit Unterschrift des Kunden ist das Projekt offiziell abgeschlossen."},
  {q:"Aus welchen Teilen besteht die IHK-Prüfung beim FISI?",o:["Nur einer schriftlichen Prüfung","AP1 nach 18 Monaten und AP2 nach 3 Jahren mit betrieblichem Projekt","Nur einem betrieblichen Projekt","Drei schriftlichen Prüfungen"],c:1,e:"AP1 (nach ~18 Monaten): schriftliche Grundlagenprüfung. AP2 (nach 3 Jahren): betriebliches Projekt mit Dokumentation und Präsentation plus schriftliche Fachprüfung."},
  {q:"Was bedeutet Eskalation im Support?",o:["Den Kunden lauter ansprechen","Ein Ticket dokumentiert an die nächsthöhere Support-Ebene übergeben","Ein Ticket löschen","Die SLA-Zeit verlängern"],c:1,e:"Kann der 1st Level nicht lösen, geht das Ticket mit allen bisherigen Erkenntnissen an den 2nd oder 3rd Level."},
  ...
  {q:"Wozu dient ein Gantt-Diagramm?",o:["Netzwerkpläne zeichnen","Die zeitliche Planung von Projektaufgaben visualisieren","Kosten abrechnen","Fehler protokollieren"],c:1,e:"Ein Gantt-Diagramm zeigt Aufgaben als Balken auf der Zeitachse – inklusive Reihenfolge und Meilensteinen."},
  {q:"Welche Pflichten hat ein Azubi nach dem BBiG?",o:["Überstunden ohne Ausgleich leisten","Lernen, sorgfältig arbeiten und Betriebsgeheimnisse wahren","Eigene Kunden akquirieren","Keine – nur der Betrieb hat Pflichten"],c:1,e:"Das BBiG verpflichtet unter anderem zum Lernen, zur Sorgfalt, zum Führen des Berichtshefts und zur Verschwiegenheit."},
  {q:"Warum steht die IST-Analyse am Anfang jedes Projekts?",o:["Um Zeit zu gewinnen","Um den tatsächlichen Zustand zu erfassen, bevor Lösungen geplant werden","Weil es das Gesetz verlangt","Um die Kosten zu erhöhen"],c:1,e:"Ohne sauberes IST kein sinnvolles SOLL: Erst verstehen, was vorhanden ist und wo es klemmt – dann lösen."},
];
...
export default function ITDart({onOpenExam}){
