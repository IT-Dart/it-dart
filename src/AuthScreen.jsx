...
export default function AuthScreen({onClose,initialMode="login",onOpenLegal}){
  ...
  const submit=async(e)=>{
    ...
    if(mode==="register"){
      ...
      if(!agbAccepted){setMsg({type:"error",text:"Bitte den AGB zustimmen."});return;}
      setBusy(true);setMsg(null);
    ...
    <div style={wrap}><div style={{...inner,paddingTop:40}}>
      ...
      <form onSubmit={submit}>
        ...
        {mode==="register"&&<label style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:14,fontSize:12,color:C.t2,lineHeight:1.6,cursor:"pointer"}}>
          ...
          <span>Ich stimme den <button type="button" onClick={()=>onOpenLegal?.("agb")} style={{background:"none",border:"none",color:C.cy,cursor:"pointer",fontSize:12,fontFamily:ff,textDecoration:"underline",padding:0}}>AGB</button> zu und habe die <button type="button" onClick={()=>onOpenLegal?.("datenschutz")} style={{background:"none",border:"none",color:C.cy,cursor:"pointer",fontSize:12,fontFamily:ff,textDecoration:"underline",padding:0}}>Datenschutzerklärung</button> zur Kenntnis genommen.</span>
        </label>}
