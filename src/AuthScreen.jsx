...
export default function AuthScreen({onClose}){
  ...
  const titles={login:"Anmelden",forgot:"Passwort zurücksetzen"};
  const subtitles={
  ...
  const buttonLabels={login:"Anmelden →",forgot:"Link senden →"};
  ...
  return(
    <div style={wrap}><div style={{...inner,paddingTop:40}}>
      ...
      <form onSubmit={submit}>
        ...
        {mode!=="forgot"&&<input type="password" placeholder="Passwort" value={password} onChange={e=>setPassword(e.target.value)} style={input} autoComplete="current-password"/>}
