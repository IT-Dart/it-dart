...
export default function MonitoringScreen({onClose}){
  ...
  const load=async()=>{
    const {data,error}=await supabase.from("monitoring_checks").select("*").order("checked_at",{ascending:false}).limit(50);
  ...
  const last24h=checks?.filter(c=>Date.now()-new Date(c.checked_at).getTime()<24*60*60*1000)??[];
  ...
  return(
    <div style={wrap}><div style={inner}>
      ...
      <div style={{background:C.s1,border:`0.5px solid ${C.bd}`,borderRadius:12,padding:"14px 16px",marginBottom:24}}>
        <p style={{fontSize:12,fontWeight:600,letterSpacing:".04em",textTransform:"uppercase",color:C.cy,marginBottom:10}}>Uptime it-dart.de (alle 5 Minuten geprüft)</p>
        {checks===null&&<p style={{fontSize:12,color:C.mu,margin:0}}>Lädt…</p>}
        {checks?.length===0&&<p style={{fontSize:12,color:C.mu,margin:0}}>Noch keine Checks — der erste Cron-Lauf steht noch aus (läuft alle 5 Minuten).</p>}
        {checks&&checks.length>0&&<>
          ...
          <p style={{fontSize:11,color:C.mu,marginTop:8,marginBottom:0}}>Jeder Balken ein Check, älteste links. Zum Detail überfahren.</p>
        </>}
      </div>
