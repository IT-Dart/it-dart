...
`rechentrainer_enabled`/`rechentrainer_until` (2026-08-06) sind das eigenständige Freischalt-Paar für den Rechentrainer (`RechentrainerScreen.jsx`, Subnetting-Übungswerkzeug) — bewusst vom Premium-Paar (`is_premium`/`premium_until`) getrennt, da es ein eigener Kaufvorgang ist, kein Premium-Bestandteil. Spiegelt exakt dasselbe Dauerhaft-ODER-befristet-Muster, Freischaltung läuft wie bei Premium über die bestehende breite Admin-RLS-Policy, keine eigene RPC. Die eigentliche Aufgaben-Generierung (`src/lib/rechentrainer/generator.js`) ist bewusst framework-frei ohne jeden Supabase-/Auth-/React-Import gehalten, um sie später unverändert in eine eigenständige PWA/Play-Store-App übernehmen zu können, ohne Rewrite.
